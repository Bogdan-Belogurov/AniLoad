...
public final class ActivityData {
    ...
    public init(size: CGSize? = nil,
        ...
        self.size = size ?? ALView.DEFAULT_BLOCKER_SIZE
        self.message = message ?? ALView.DEFAULT_BLOCKER_MESSAGE
        self.messageFont = messageFont ?? ALView.DEFAULT_BLOCKER_MESSAGE_FONT
        self.messageSpacing = messageSpacing ?? ALView.DEFAULT_BLOCKER_MESSAGE_SPACING
        self.type = type ?? ALView.DEFAULT_TYPE
        self.color = color ?? ALView.DEFAULT_COLOR
        self.padding = padding ?? ALView.DEFAULT_PADDING
        self.displayTimeThreshold = displayTimeThreshold ?? ALView.DEFAULT_BLOCKER_DISPLAY_TIME_THRESHOLD
        self.minimumDisplayTime = minimumDisplayTime ?? ALView.DEFAULT_BLOCKER_MINIMUM_DISPLAY_TIME
        self.backgroundColor = backgroundColor ?? ALView.DEFAULT_BLOCKER_BACKGROUND_COLOR
        self.textColor = textColor ?? color ?? ALView.DEFAULT_TEXT_COLOR
    }
}

private protocol ALPresenterState {
    func startAnimating(presenter: ALPresenter, _ fadeInAnimation: FadeInAnimation?)
    func stopAnimating(presenter: ALPresenter, _ fadeOutAnimation: FadeOutAnimation?)
}

private struct ALPresenterStateWaitingToStart: ALPresenterState {
    func startAnimating(presenter: ALPresenter, _ fadeInAnimation: FadeInAnimation?) {
...
private struct ALPresenterStateAnimating: ALPresenterState {
    func startAnimating(presenter: ALPresenter, _ fadeInAnimation: FadeInAnimation?) {
        // Do nothing
    }
...
private struct ALPresenterStateWaitingToStop: ALPresenterState {
    func startAnimating(presenter: ALPresenter, _ fadeInAnimation: FadeInAnimation?) {
...
private struct ALPresenterStateStopped: ALPresenterState {
    func startAnimating(presenter: ALPresenter, _ fadeInAnimation: FadeInAnimation?) {
        guard let activityData = presenter.data else { return }
...
public final class ALPresenter {
    fileprivate enum State: ALPresenterState {
        case waitingToStart
        case animating
        case waitingToStop
        case stopped
        
        var performer: ALPresenterState {
            switch self {
            case .waitingToStart: return ALPresenterStateWaitingToStart()
            case .animating: return ALPresenterStateAnimating()
            case .waitingToStop: return ALPresenterStateWaitingToStop()
            case .stopped: return ALPresenterStateStopped()
            }
        }
    ...
    private let restorationIdentifier = "ALViewContainer"
    ...
    public static let sharedInstance = ALPresenter()
    ...
    fileprivate func show(with activityData: ActivityData, _ fadeInAnimation: FadeInAnimation?) {
        ...
        let activityIndicatorView = ALView(
